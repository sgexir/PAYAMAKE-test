(() => {
  ...
  function list(items,labelKey){
    ...
    return `<div class="web-runtime-list">${items.map(x=>`<div class="web-runtime-row"><span>${esc(x[labelKey]||'—')}</span><strong class="web-runtime-count">${num(x.pageViews)}</strong></div>`).join('')}</div>`;
  }

  async function load(){
    const box=$('#webAnalyticsContent'); if(!box)return;
    box.innerHTML='<div class="web-runtime-loading">در حال دریافت آمار کامل بازدید سایت...</div>';
    try{
      ...
      box.innerHTML=`<div class="web-runtime-summary"><div class="web-runtime-stat"><span>بازدید صفحات (Page Views)</span><strong>${num(s.pageViews)}</strong></div><div class="web-runtime-stat"><span>Visits</span><strong>${num(s.visits)}</strong></div><div class="web-runtime-stat"><span>صفحات ثبت‌شده</span><strong>${num((d.topPages||[]).length)}</strong></div><div class="web-runtime-stat"><span>کشورهای شناسایی‌شده</span><strong>${num((d.countries||[]).length)}</strong></div></div><div class="web-runtime-chart"><h4>روند بازدید سایت</h4>${chart(series)}</div><div class="web-runtime-columns"><div class="web-runtime-panel"><h4>صفحات پربازدید</h4>${list(d.topPages,'path')}</div><div class="web-runtime-panel"><h4>کشورها</h4>${list(d.countries,'country')}</div><div class="web-runtime-panel"><h4>نوع دستگاه</h4>${list(d.devices,'device')}</div><div class="web-runtime-panel"><h4>مرورگر</h4>${list(d.browsers,'browser')}</div><div class="web-runtime-panel"><h4>سیستم‌عامل</h4>${list(d.operatingSystems,'os')}</div><div class="web-runtime-panel"><h4>منابع ورود (Referrer)</h4>${list(d.referrers,'referrer')}</div></div><div class="web-runtime-note">IP خام کاربران در پنل ذخیره یا نمایش داده نمی‌شود؛ اطلاعات جغرافیایی و فنی به‌صورت تجمیعی از Cloudflare Analytics دریافت می‌شوند.</div><div class="web-runtime-meta">سایت: ${esc(d.site||'payamake.ir')} · منبع: Cloudflare HTTP Analytics (eyeball traffic) · بروزرسانی: ${new Date().toLocaleString('fa-IR')}</div>`;
  ...
  function inject(){
    ...
    card.innerHTML='<div class="sms-section-title"><div><h3>بازدید و ترافیک سایت</h3><span>آمار واقعی بازدید صفحات و رفتار کلی ترافیک سایت PAYAMAKE.</span></div><button id="webAnalyticsRefresh" class="admin-button outline small" type="button">بروزرسانی</button></div><div class="web-runtime-toolbar"><div class="web-runtime-ranges"><button data-web-days="1" type="button">امروز</button><button data-web-days="3" type="button">۳ روز</button><button data-web-days="7" type="button">۷ روز</button><button data-web-days="30" class="active" type="button">۳۰ روز</button><button data-web-days="90" type="button">۹۰ روز</button><button data-web-days="180" type="button">۱۸۰ روز</button></div><span>Cloudflare HTTP Analytics</span></div><div id="webAnalyticsContent" class="web-runtime-loading">در حال آماده‌سازی...</div>';
