...
export async function refreshPendingSmsDeliveries(env) {
  ...
  const rows = await env.DB.prepare(`SELECT l.*, p.provider_key, p.is_enabled FROM sms_logs l JOIN sms_providers p ON p.id = l.provider_id WHERE l.send_status = 'sent' AND l.delivery_status IN ('pending', 'unknown') AND l.provider_message_id IS NOT NULL ORDER BY l.created_at ASC LIMIT 100`).all();
  let updated = 0; const errors = []; const byProvider = {};
  for (const row of rows.results || []) {
    const key = String(row.provider_key || "unknown");
    byProvider[key] = byProvider[key] || { checked: 0, updated: 0, errors: 0 }; byProvider[key].checked++;
    try {
      ...
      await updateSmsLog(env.DB, row.id, { deliveryStatus: report.deliveryStatus, providerStatus: report.providerStatus || null, providerCode: report.providerCode == null ? row.provider_code : String(report.providerCode), providerResponse: report.raw, deliveredAt: report.deliveryStatus === "delivered" ? new Date().toISOString() : null });
      updated++; byProvider[key].updated++;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error); byProvider[key].errors++; errors.push({ id: row.id, provider: key, message }); console.error("SMS delivery refresh error:", row.id, key, error);
    }
  }
...
async function sendNiazpardaz(env, recipient, sender, message) { if (!env.NIAZPARDAZ_API_KEY) throw new Error("NIAZPARDAZ_API_KEY تنظیم نشده است."); if (!sender) throw new Error("NIAZPARDAZ_SENDER_NUMBER تنظیم نشده است."); if (!message) throw new Error("متن پیامک نیازپرداز تنظیم نشده است."); const response = await fetch(`${NIAZPARDAZ_BASE_URL}/SendBatchSms`, { method: "POST", headers: { "Content-Type": "application/json", "X-API-Key": env.NIAZPARDAZ_API_KEY }, body: JSON.stringify({ fromNumber: sender, messageContent: message, toNumbers: recipient, isFlash: false }) }); const text = await response.text(), raw = parseJson(text), result = raw?.result || raw; const code = result?.resultCode ?? raw?.resultCode ?? null; const success = response.ok && raw?.success !== false && Number(code) === 0; return { success, status: response.status, providerMessageId: result?.batchSmsId ?? result?.batchSMSId ?? null, providerCode: code, providerStatus: success ? "accepted" : "rejected", raw, errorMessage: success ? null : (raw?.errorMessage || `Niazpardaz result code: ${code}`) }; }
async function getNiazpardazDelivery(env, batchSmsId) { if (!env.NIAZPARDAZ_API_KEY) return { hasStatus: false, errorMessage: "NIAZPARDAZ_API_KEY تنظیم نشده است." }; const response = await fetch(`${NIAZPARDAZ_BASE_URL}/GetBatchDelivery`, { method: "POST", headers: { "Content-Type": "application/json", "X-API-Key": env.NIAZPARDAZ_API_KEY }, body: JSON.stringify({ batchSmsId: Number(batchSmsId), index: 1, count: 100 }) }); const text = await response.text(), raw = parseJson(text), result = raw?.result || raw; if (!response.ok || raw?.success === false) return { hasStatus: false, raw, errorMessage: raw?.errorMessage || `Niazpardaz Delivery HTTP ${response.status}` }; const statuses = result?.deliveryStatus || result?.deliveryStatuses || []; const status = Array.isArray(statuses) ? Number(statuses[0]) : Number(statuses); const map = { 1: "delivered", 2: "failed", 3: "failed", 7: "failed", 9: "failed", 10: "failed", 11: "failed", 13: "pending", 0: "pending", 5: "pending", 6: "failed", 8: "unknown", 12: "failed" }; return { hasStatus: true, deliveryStatus: map[status] || "unknown", providerStatus: String(status), providerCode: result?.resultCode ?? raw?.resultCode ?? null, raw }; }
async function sendSmsIr(env, recipient, templateId, parameters) { if (!env.SMSIR_API_KEY) throw new Error("SMSIR_API_KEY تنظیم نشده است."); const id = Number(templateId || 0); if (!id) throw new Error("SMS.ir template ID تنظیم نشده است."); const response = await fetch(SMSIR_VERIFY_URL, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json", "x-api-key": env.SMSIR_API_KEY }, body: JSON.stringify({ mobile: recipient, templateId: id, parameters: parameters || [] }) }); const text = await response.text(), raw = parseJson(text), messageId = raw?.data?.messageId ?? raw?.data?.messageID ?? null; return { success: response.ok, status: response.status, providerMessageId: messageId, providerCode: raw?.status ?? response.status, providerStatus: response.ok ? "accepted" : "rejected", raw, errorMessage: response.ok ? null : (raw?.message || `SMS.ir HTTP ${response.status}`) }; }
...
async function getSmsIrDelivery(env, messageId) {
  ...
  const normalizedId = String(messageId ?? "").trim().replace(/\.0+$/, "");
  if (!/^\d+$/.test(normalizedId)) return { hasStatus: false, errorMessage: `SMS.ir messageId نامعتبر است: ${messageId}` };
