...
async function getProvider(db, providerKey) {
  return db.prepare(`SELECT * FROM sms_providers WHERE provider_key = ? LIMIT 1`).bind(providerKey).first();
}

async function getTemplate(db, providerId, purpose) {
  return db.prepare(`SELECT * FROM sms_templates WHERE provider_id = ? AND purpose = ? AND is_enabled = 1 ORDER BY is_default DESC, id ASC LIMIT 1`).bind(providerId, purpose).first();
}

async function createSmsLog(db, data) {
  const result = await db.prepare(`
    INSERT INTO sms_logs (provider_id, lead_id, purpose, recipient, sender, template_id, message, send_status, delivery_status, created_at, updated_at)
...
async function sendNiazpardaz(env, recipient, sender, message) {
  ...
  const response = await fetch(`${NIAZPARDAZ_BASE_URL}/SendBatchSms`, {
    method: "POST", headers: { "Content-Type": "application/json", "X-API-Key": env.NIAZPARDAZ_API_KEY },
...
async function sendSmsIr(env, recipient, templateId, parameters) {
  ...
  const response = await fetch(SMSIR_VERIFY_URL, {
    method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json", "x-api-key": env.SMSIR_API_KEY },
    body: JSON.stringify({ mobile: recipient, templateId: id, parameters: parameters || [] }),
  });
...
function getEnvTemplateRef(env, providerKey, purpose) {
  if (providerKey !== "sms_ir") return null;
  return purpose === "lead_customer" ? env.SMSIR_CUSTOMER_TEMPLATE_ID : env.SMSIR_ADMIN_TEMPLATE_ID;
}
...
function renderTemplate(template, parameters) {
  ...
  for (const parameter of parameters || []) {
    ...
    if (name) text = text.replaceAll(`{${name}}`, String(parameter?.value ?? ""));
  }
  return text;
}
