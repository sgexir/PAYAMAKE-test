...
export async function sendLeadSms({ env, db, leadId, recipient, purpose, parameters, message }) {
  ...
  const logId = await createSmsLog(db, {
    ...
    templateId: template?.template_ref || null,
    message: renderedMessage,
  });

  try {
    const result = providerKey === "sms_ir"
      ? await sendSmsIr(env, recipient, template, parameters)
      : await sendNiazpardaz(env, recipient, provider.sender_number, renderedMessage);
    ...
    await updateSmsLog(db, logId, {
      ...
      deliveryStatus: result.success ? "pending" : "unknown",
    });
    ...
    await updateSmsLog(db, logId, {
      sendStatus: "failed",
      providerResponse: { exception: messageText },
...
export async function refreshPendingSmsDeliveries(env) {
  if (!env.DB) return { checked: 0, updated: 0 };
  const rows = await env.DB.prepare(`
    SELECT l.*, p.provider_key, p.is_enabled
    FROM sms_logs l
    JOIN sms_providers p ON p.id = l.provider_id
    WHERE l.send_status = 'sent'
      AND l.delivery_status IN ('pending', 'unknown')
  ...
  for (const row of rows.results || []) {
    try {
      const report = row.provider_key === "sms_ir"
        ? await getSmsIrDelivery(env, row.provider_message_id)
        : await getNiazpardazDelivery(env, row.provider_message_id);
      if (!report || !report.hasStatus) continue;
      ...
      console.error("SMS delivery refresh error:", row.id, error);
    }
  }
...
async function getProvider(db, providerKey) {
  return (await db.prepare(`SELECT * FROM sms_providers WHERE provider_key = ? LIMIT 1`).bind(providerKey).first());
}

async function getTemplate(db, providerId, purpose) {
...
async function createSmsLog(db, data) {
  const result = await db.prepare(`
    INSERT INTO sms_logs (
      provider_id, lead_id, purpose, recipient, sender, template_id, message,
      send_status, delivery_status, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', 'unknown', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
  `).bind(
    data.providerId, data.leadId || null, data.purpose, data.recipient, data.sender || null,
    data.templateId || null, data.message || null
  ).run();
...
async function updateSmsLog(db, id, data) {
  ...
  const values = [];
  const map = {
    sendStatus: "send_status", deliveryStatus: "delivery_status", providerMessageId: "provider_message_id",
    providerCode: "provider_code", providerStatus: "provider_status", providerResponse: "provider_response",
    errorMessage: "error_message", sentAt: "sent_at", deliveredAt: "delivered_at"
  };
...
async function sendNiazpardaz(env, recipient, sender, message) {
  ...
  const response = await fetch(`${NIAZPARDAZ_BASE_URL}/SendBatchSms`, {
    ...
    body: JSON.stringify({ fromNumber: sender, messageContent: message, toNumbers: recipient, isFlash: false }),
  });
  const text = await response.text();
  const raw = parseJson(text);
  const result = raw?.result || raw;
  const success = response.ok && raw?.success !== false && Number(result?.resultCode ?? raw?.resultCode ?? -1) === 0;
  const code = result?.resultCode ?? raw?.resultCode ?? null;
  return {
    success,
    status: response.status,
    providerMessageId: result?.batchSmsId ?? result?.batchSMSId ?? null,
    providerCode: code,
    providerStatus: success ? "accepted" : "rejected",
    raw,
    errorMessage: success ? null : (raw?.errorMessage || `Niazpardaz result code: ${code}`),
  };
}

async function getNiazpardazDelivery(env, batchSmsId) {
  if (!env.NIAZPARDAZ_API_KEY) return null;
  const response = await fetch(`${NIAZPARDAZ_BASE_URL}/GetBatchDelivery`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-API-Key": env.NIAZPARDAZ_API_KEY },
    body: JSON.stringify({ batchSmsId: Number(batchSmsId), index: 1, count: 100 }),
  });
  const text = await response.text();
  const raw = parseJson(text);
  const result = raw?.result || raw;
  if (!response.ok || raw?.success === false) return { hasStatus: false, raw };
...
async function sendSmsIr(env, recipient, template, parameters) {
  if (!env.SMSIR_API_KEY) throw new Error("SMSIR_API_KEY تنظیم نشده است.");
  const templateId = Number(template?.template_ref || 0);
  if (!templateId) throw new Error("SMS.ir template ID تنظیم نشده است.");
  const response = await fetch(SMSIR_VERIFY_URL, {
    ...
    body: JSON.stringify({ mobile: recipient, templateId, parameters: parameters || [] }),
  });
  const text = await response.text();
  const raw = parseJson(text);
  const success = response.ok;
  const messageId = raw?.data?.messageId ?? raw?.data?.messageID ?? null;
  return {
    success,
    status: response.status,
    providerMessageId: messageId,
    providerCode: raw?.status ?? response.status,
    providerStatus: success ? "accepted" : "rejected",
    raw,
    errorMessage: success ? null : (raw?.message || `SMS.ir HTTP ${response.status}`),
  };
}

async function getSmsIrDelivery(env, messageId) {
  if (!env.SMSIR_API_KEY) return null;
  const response = await fetch(`${SMSIR_REPORT_URL}${encodeURIComponent(messageId)}`, {
    headers: { Accept: "application/json", "x-api-key": env.SMSIR_API_KEY },
  });
  const text = await response.text();
  const raw = parseJson(text);
  if (!response.ok) return { hasStatus: false, raw };
  const data = raw?.data || raw;
  const state = data?.deliveryState;
  if (state == null) return { hasStatus: false, raw };
...
function renderTemplate(template, parameters) {
  ...
  for (const parameter of parameters || []) {
    ...
    if (!name) continue;
    text = text.replaceAll(`{${name}}`, String(parameter?.value ?? ""));
  }
  return text;
}
