...
async function updateProvider(request, db) {
  ...
  if (body.senderNumber !== undefined) {
    await db.prepare(`UPDATE sms_providers SET sender_number = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(String(body.senderNumber || "").trim() || null, id).run();
  }
...
async function updateTemplate(request, db) {
  ...
  if (body.variablesJson !== undefined) {
    const variables = String(body.variablesJson || "[]").trim() || "[]";
    try { JSON.parse(variables); } catch { return json({ success: false, error: "Variables JSON معتبر نیست." }, 400); }
    fields.push("variables_json = ?"); values.push(variables);
  }
