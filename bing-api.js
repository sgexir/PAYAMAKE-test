...
async function startOAuth(request, env, admin) {
  ...
  ).bind(stateHash, admin.admin_id).run();
...
function getRedirectUri(request) { const url = new URL(request.url); return `${url.origin}${REDIRECT_PATH}`; }
function getAppRedirectBase(request) { const origin = request.headers.get("Origin"); return origin === "https://staging.payamake.ir" ? origin : "https://staging.payamake.ir"; }
...
async function encryptToken(secret, value) { if (!secret) throw new Error("AUTH_ENCRYPTION_KEY تنظیم نشده است."); const key = await deriveKey(secret); const iv = crypto.getRandomValues(new Uint8Array(12)); const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, new TextEncoder().encode(value)); return `${bytesToHex(iv)}.${bytesToBase64(new Uint8Array(encrypted))}`; }
