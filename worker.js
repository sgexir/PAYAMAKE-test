  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/admin/" || url.pathname === "/admin/index.html") {
      ...
      if (contentType.includes("text/html")) {
        ...
        const adminScripts = '<script src="../js/admin-analytics.js?v=9"></script><script src="../js/admin-cloudflare.js?v=9"></script><script src="../js/admin-web-analytics.js?v=1"></script><script src="../js/admin-google-search.js?v=1"></script><script src="../js/security-center-bootstrap.js?v=2"></script><script src="../js/security-center.js?v=8"></script>';
    ...
    if (url.pathname.startsWith("/api/admin/leads")) { const response = await handleLeadsApi(request, env); return withCors(response, origin); }
    if (url.pathname.startsWith("/api/admin/sms/") || url.pathname.startsWith("/api/admin/system/")) { const response = await handleAdminApi(request, env); return withCors(response, origin); }
    if (url.pathname.startsWith("/api/admin/")) { const response = await handleAdminAuth(request, env); return withCors(response, origin); }
    ...
    try {
      ...
      let body; try { body = await request.json(); } catch { return jsonResponse({ success: false, error: "داده ارسالی معتبر نیست." }, 400, origin); }
      const fullName = String(body.fullName || "").trim(), phone = String(body.phone || "").trim(), brand = String(body.brand || "").trim(), type = String(body.type || "").trim(), description = String(body.description || "").trim(), source = String(body.source || "homepage").trim();
      ...
      const normalizedPhone = normalizeIranMobile(phone); if (!normalizedPhone) return jsonResponse({ success: false, error: "شماره موبایل معتبر نیست." }, 400, origin);
      const adminMobile = normalizeIranMobile(env.ADMIN_MOBILE); if (!adminMobile) return jsonResponse({ success: false, error: "ADMIN_MOBILE معتبر نیست." }, 500, origin);
      const safeFullName = limitForPattern(fullName), safeBrand = limitForPattern(brand), safeType = limitForPattern(type), safeDescription = limitForPattern(description);
      ...
      const leadId = insertResult.meta?.last_row_id; if (!leadId) return jsonResponse({ success: false, error: "ذخیره درخواست انجام نشد." }, 500, origin);
...
async function runDeliveryCron(env, event) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error); console.error("SMS delivery cron failed:", error);
...
async function ensureSystemLogsTable(db) { if (!db) return; await db.prepare(`CREATE TABLE IF NOT EXISTS system_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, level TEXT NOT NULL DEFAULT 'info', source TEXT NOT NULL, event TEXT, message TEXT, details_json TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`).run(); }
async function writeSystemLog(db, level, source, message, details = null) { if (!db) return; await db.prepare(`INSERT INTO system_logs (level, source, event, message, details_json) VALUES (?, ?, ?, ?, ?)`).bind(level, source, source, message, details == null ? null : JSON.stringify(details)).run(); }
async function updateSmsStatus(db, leadId, column, status) { const allowedColumns = ["customer_sms_status", "admin_sms_status"]; if (!allowedColumns.includes(column)) throw new Error("Invalid SMS status column."); await db.prepare(`UPDATE leads SET ${column} = ? WHERE id = ?`).bind(status, leadId).run(); }
function normalizeIranMobile(value) { let phone = String(value || "").trim().replace(/\s+/g, "").replace(/-/g, ""); if (phone.startsWith("+98")) phone = "0" + phone.slice(3); else if (phone.startsWith("98") && phone.length === 12) phone = "0" + phone.slice(2); return /^09\d{9}$/.test(phone) ? phone : null; }
